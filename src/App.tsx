...
function FormationPreview({
  ...
  // base points: [0]=GK, others=field slots (4)
  const pts = FORMATION_POINTS[formation];
  const baseFieldPts = pts.slice(1);

  // If we have 5 field players, expand to 5 field slots depending on formation
  const fiveFieldPts: {x:number;y:number;label:string}[] = (() => {
    if (fields.length < 5) return baseFieldPts;
    switch (formation) {
      case "1-2-1":
        // DF center, MF L/C/R, FW center
        return [
          { x: 50, y: 90, label: "DF" },
          { x: 30, y: 58, label: "MF" },
          { x: 50, y: 58, label: "MF" },
  ...
  const coordsFields = fiveFieldPts;
  ...
  // Helper for display name inside jersey (뒤 3글자)
  ...
  const showMercenaryLabel = fields.length === 3 && coordsFields.length === 4;

  // Precompute name map
  const nameOf = (pid: string) => players.find(p => p.id === pid)?.name || "?";

  // Build "used" set (for bench/GK label computation later)
    ...
    <div className="formation-card">
      ...
      <svg viewBox="0 0 100 140" className="pitch">
        ...
        {coordsFields.map((pt, i) => {
          ...
          const displayTxt = pid
            ? tail3Local(nameOf(pid))
            : (showMercenaryLabel ? "용병" : "?");
          return (
            <g key={i} transform={`translate(${pt.x}, ${pt.y})`} textAnchor="middle">
              <UniformIcon fill={jerseyFill} size={20} stroke="var(--jersey-stroke)" />
              <text className="player-initials" dominantBaseline="middle" dy="0.3em">{displayTxt}</text>
            </g>
          );
        })}

        {/* GK: show at bottom only when exactly one GK */}
        {singleGK && (
      ...
      {/* If GK is 2, replace bench with "GK:" list; do not show the normal bench block */}
      {gkIds.length === 2 ? (
        <div className="bench">GK: {gkIds.map(id => nameOf(id)).join(", ")}</div>
      ) : (
        (() => {
          // Normal bench: anyone not used (excluding when 2 GKs scenario above)
          const benchNames = roster.filter(id => !usedSet.has(id)).map(nameOf);
          return benchNames.length > 0 ? <div className="bench">벤치: {benchNames.join(", ")}</div> : null;
        })()
      )}
    </div>
  );
