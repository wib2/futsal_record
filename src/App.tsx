...
function isEmptyLike(x: any) {
  try {
    const playersCnt = Array.isArray(x?.players) ? x.players.length : 0;
    const sessions = x?.sessionsByDate || {};
    const hasAnyRoster = Object.values(sessions).some((sess: any) =>
      Object.values(sess?.rosters || {}).some((arr: any) => Array.isArray(arr) ? arr.length > 0 : !!arr?.length)
    );
    const hasAnyMatch = Object.values(sessions).some((sess: any) =>
      Array.isArray(sess?.matches) ? sess.matches.length > 0 : !!sess?.matches?.length
    );
    return playersCnt === 0 && !hasAnyRoster && !hasAnyMatch;
  } catch {
    return false;
  }
}
...
function FormationPreview({
  ...
  // 5인 확장: 필드 플레이어가 5명일 때 포메이션에 맞게 5개 좌표로 확장
  if (fields.length >= 5) {
    switch (formation) {
      case "1-2-1":
        coordsFields = [
...
function exportCurrentState() {
  ...
  useEffect(() => {
    ...
    debTimerRef.current = setTimeout(() => {
      const cloudEmpty = isEmptyLike(cloud);
      const nextEmpty = isEmptyLike(s);
      if (!(nextEmpty && !cloudEmpty) && JSON.stringify(pickState(cloud)) !== JSON.stringify(pickState(s))) setCloud(s);
    }, 150);
  ...
  function exportDataToFile(data: any) {
  try {
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json;charset=utf-8" });
    const ts = new Date();
    const pad = (n:number)=>String(n).padStart(2,"0");
    const name = `goldin-backup-${ts.getFullYear()}${pad(ts.getMonth()+1)}${pad(ts.getDate())}-${pad(ts.getHours())}${pad(ts.getMinutes())}.json`;
...
function copyViewerLink() { const url = new URL(window.location.href); url.searchParams.set("viewer", "1"); navigator.clipboard?.writeText(url.toString()); alert("보기 전용 링크가 복사되었습니다"); }
  ...
  const addMatch = () => {
    ...
    let maxSeq = 0;
    asArray(cur.matches, []).forEach(m => { if (m.seq && m.seq > maxSeq) maxSeq = m.seq; });
    const nextSeq = maxSeq + 1;

    // Round-robin base pairs: [A-B], [B-C], [C-A]
    const basePairs: [TeamId, TeamId][] = [["A","B"], ["B","C"], ["C","A"]];
    const idx = (nextSeq - 1) % basePairs.length;
    const cycle = Math.floor((nextSeq - 1) / basePairs.length);

    let [home, away] = basePairs[idx];
    // Alternate home/away every cycle for fairness
    if (cycle % 2 === 1) { const t = home; home = away; away = t; }

    // Auto-pick GK when a team has exactly 1 GK in its roster; otherwise leave null
    const pickOneGK = (tid: TeamId): string | null => {
      const ids = asArray(cur.rosters[tid], []);
      const gkIds = ids.filter((pid: string) => (players.find(p => p.id === pid)?.pos) === "GK");
      return gkIds.length === 1 ? gkIds[0] : null;
    };
    const gkHome = pickOneGK(home);
    const gkAway = pickOneGK(away);

    patchSession({ matches: [
      ...asArray(cur.matches, []),
      { id: uid(), seq: nextSeq, home, away, hg: 0, ag: 0, gkHome: gkHome ?? null, gkAway: gkAway ?? null }
    ] });
  };
      ...
      <div className="panel">
        ...
        {authed ? <>
          ...
          <button onClick={copyViewerLink}>보기 전용 링크 복사</button>
            <button onClick={exportCurrentState}>데이터 내보내기</button>
        </> : <>
