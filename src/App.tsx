      ...
      <div className="match-head">
        ...
        <div className="scoreline">
          <select value={m.home}
            ...
            disabled={readonly}
          >
            {getActiveTeams(cur?.hasTeamD || false).map(tid => (
              <option key={tid} value={tid}>{teamNames[tid]}</option>
            ))}
          </select>
...
export default function App() {
  ...
  const local0 = loadLocal() ?? {
    ...
    teamNames: { A: "팀 A", B: "팀 B", C: "팀 C", D: "팀 D" },
    sessionsByDate: { [today]: emptySession() },
    sessionDate: today
  };
...
useEffect(() => {
  ...
  function calcScores(session: Session) {
    ...
    const teamBonusByTeam = computeTeamBonus(standings.filter(r => getActiveTeams(cur?.hasTeamD || false).includes(r.team)), (session as any)?.hasTeamD || false);
        ...
        <>
          ...
          <section className="box">
            ...
            <div className="table-wrap">
              <h4>순위표 (팀 보너스: {cur?.hasTeamD ? "1위 4 / 2위 3 / 3위 2 / 4위 1" : "1위 4 / 2위 2 / 3위 1"})</h4>
              <table className="tbl"><thead><tr>
                ...
                {computeStandings(cur.matches).map((t, idx) => (
                  <tr key={t.team}><td>{idx + 1}</td><td>{effectiveTeamNames[t.team]} <span className="subtle">({t.team})</span></td><td>{t.pts}</td><td>{t.w}</td><td>{t.d}</td><td>{t.l}</td><td>{t.gf}</td><td>{t.ga}</td><td>{t.gd}</td><td className="bold">{(Array.isArray(cur.matches) && cur.matches.length>0 ? computeTeamBonus(computeStandings(cur.matches).filter(r => getActiveTeams(cur?.hasTeamD || false).includes(r.team)), cur?.hasTeamD || false)[t.team] : 0)}</td></tr>
                ))}
              </tbody></table>
            </div>
          </section>
