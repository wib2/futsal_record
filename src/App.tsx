...
export default function App() {
  ...
  // ✅ 저장/동기화: 저장 시 보정하지 않고, 동일 상태면 setCloud 호출 안 함
  useEffect(() => {
    const s = { players, teamNames: globalTeamNames, sessionsByDate, sessionDate };
    saveLocal(s);
    if (ready) {
      if (!cloud || !deepEqual(cloud, s)) {
        setCloud(s);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [players, globalTeamNames, sessionsByDate, sessionDate, ready]); // cloud를 의존성에서 제외
  ...
  const patchSession = (patch: Partial<Session>) => {
    ...
    setSessionsByDate(prev => {
      ...
      // 동일 체크로 불필요 전파 방지
      if (deepEqual(base, next)) return prev;
