...
type FormationKey = "1-2" | "1-2-1" | "2-2-1" | "2-2" | "3-1";
...
type Session = {
  ...
  teamColors?: Record<TeamId, "red" | "yellow" | "white">; /* ✅ 팀별 조끼 색상 */
};
type PersistShape = {
  players: Player[];
  teamNames: Record<TeamId, string>;
...
function emptySession(): Session {
  return {
    ...
    teamColors: { A: "red", B: "yellow", C: "white" },
  };
}
...
const FORMATION_POINTS: Record<FormationKey, { x: number; y: number; label: string }[]> = {
  ...
  "2-2-1": [
    ...
    { x: 35, y: 44, label: "MF" }, { x: 65, y: 44, label: "MF" },
    { x: 50, y: 20, label: "FW" },
  ],
  "2-2": [
    { x: 50, y: 92, label: "GK" },
...
function FormationPreview({
  ...
  /* 5명: GK 1 + 필드 4 자동 선정 */
  const gks = roster.filter(id => players.find(p => p.id === id)?.pos === "GK");
  const fields = roster.filter(id => players.find(p => p.id === id)?.pos !== "GK");
  const hasGK = gks.length === 1;
  const gk = hasGK ? gks[0] : null;
  // 필드 인원 수에 따라 포메이션 자동 선택
  const fCount = fields.length;
  let autoForm: FormationKey = formation;
  if (fCount === 3) autoForm = "1-2";
  else if (fCount === 4) autoForm = "1-2-1";
  else if (fCount >= 5) autoForm = "2-2-1";
  const pts = FORMATION_POINTS[autoForm];
  const coords = hasGK ? pts : pts.filter(p => p.label !== "GK");

  const order = (hasGK ? [gk, ...fields] : [...fields]);
  const chosen = order.filter(Boolean).slice(0, pts.length) as string[];
...
<select
  value={cur.teamColors?.[tid] ?? TEAM_PINNIES[tid]}
  onChange={e => patchSession({ teamColors: { ...(cur.teamColors||{}), [tid]: e.target.value as any } })}
  disabled={readonly}
  className="inline"
  style={{ marginRight: 8 }}
>
...
<span className="hint">조끼색: {TEAM_PINNIES[tid] === "red" ? "빨강" : TEAM_PINNIES[tid] === "yellow" ? "노랑" : "하양"}</span>
                    ...
                    <FormationPreview
                      ...
                      formation={formation}
                      color={cur.teamColors?.[tid] ?? TEAM_PINNIES[tid]}
                    />
