...
type PinnieColor = "red" | "yellow" | "white";
...
type Session = {
  ...
  teamColors: Record<TeamId, PinnieColor>;
  ng | null>;
...
function emptySession(): Session {
  return {
    rosters: { A: [], B: [], C: [] },
    teamColors: { A: "red", B: "yellow", C: "white" } as Record<TeamId, PinnieColor>,
    matches: [],
    matchStats: {},
...
function normalizeLoaded(data: any): PersistShape {
  ...
  if (data?.sessionsByDate && typeof data.sessionsByDate === "object") {
    for (const [k, v] of Object.entries<any>(data.sessionsByDate)) {
      ...
      const notes =
      const teamColors: Record<TeamId, PinnieColor> = {
        A: (s?.teamColors?.A as PinnieColor) || "red",
        B: (s?.teamColors?.B as PinnieColor) || "yellow",
        C: (s?.teamColors?.C as PinnieColor) || "white",
      };
      ...
      sessionsByDate[ensureSunday(k)] = { rosters, matches, matchStats, defAwards, teamColors, teamNames, notes, rosterViewConfirmed, formations };
    }
  } else {
  ...
  Object.values(sessionsByDate).forEach(sess => {
    ...
    if (!(sess as any).teamColors) (sess as any).teamColors = { A: "red", B: "yellow", C: "white" };
  });
...
,
  "1-2": [
    { x: 50, y: 92, label: "GK" },
    { x: 50, y: 58, label: "MF" },
    { x: 30, y: 30, label: "FW" }, { x: 70, y: 30, label: "FW" }
  ],
  "2-2-1": [
    { x: 50, y: 92, label: "GK" },
    { x: 25, y: 72, label: "DF" }, { x: 75, y: 72, label: "DF" },
    { x: 30, y: 48, label: "MF" }, { x: 70, y: 48, label: "MF" },
    { x: 50, y: 24, label: "FW" }
  ],
}; 
...
function FormationPreview({
  ...
  /* 5명: GK 1 + 필드 4 자동 선정 */
  const gkList = roster.filter(id => players.find(p => p.id === id)?.pos === "GK");
  const uniqueGK = gkList.length === 1 ? gkList[0] : null;
  const fields = roster.filter(id => players.find(p => p.id === id)?.pos !== "GK");

  // Decide formation automatically by field player count
  let autoFormation: FormationKey = formation;
  if (fields.length === 3) autoFormation = "1-2";
  else if (fields.length === 4) autoFormation = "1-2-1";
  else if (fields.length >= 5) autoFormation = "2-2-1";

  const ptsAll = FORMATION_POINTS[autoFormation] || FORMATION_POINTS[formation];

  const includeGK = Boolean(uniqueGK);
  const fieldSlots = ptsAll.filter(p => p.label !== "GK").length;
  const chosenFields = fields.slice(0, fieldSlots);
  const chosen = (includeGK ? [uniqueGK, ...chosenFields] : chosenFields) as string[];

  /* 좌표: GK + 4 */
  const pts = FORMATION_POINTS[formation];
  const coords = pts.length === 4 ? pts : pts; // 타입 만족용
        ...
        {coords.map((pt, i) => {
          ...
          return (
            <g key={i} transform={`translate(${pt.x}, ${pt.y})`}>
              ...
              <g className="jersey">
                <g transform="scale(2)">
                  <path d="M-5,0 L-8,-3 L-6,-6 L-3,-4 L3,-4 L6,-6 L8,-3 L5,0 L5,7 L-5,7 Z"
                        fill={jerseyFill} stroke="var(--jersey-stroke)" strokeWidth="0.6" />
                </g>
              </g>
              <text textAnchor="middle" dominantBaseline="middle" y="2.0" style={{ fontSize: 4.2, fontWeight: 800 }}>
                {name ? name.slice(-2) : ""}
              </text>
              <text y="10.5" textAnchor="middle" className="player-label" style={{ fontSize: 3.2 }}>{pid ? name : "빈 자리"}</text>
            </g>
          );
        })}
      </svg>
      {roster.length > chosen.length && (
        <div className="bench">벤치: {roster.filter(id => !chosen.includes(id)).map(id => (players.find(p => p.id === id)?.name || "?")).join(", ")}</div>
      )}
    </div>
  );
}
...
export default function App() {
  ...
  const setTeamColor = (tid: TeamId, c: PinnieColor) =>
    patchSession({ teamColors: { ...((cur as any).teamColors || { A: "red", B: "yellow", C: "white" }), [tid]: c } as any });
                ...
                return (
                  <div key={tid} className="team-card">
                    ...
                    <FormationPreview
                      ...
                      formation={formation}
                      color={(cur as any).teamColors?.[tid] || TEAM_PINNIES[tid]}
                    />
