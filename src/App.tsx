 ...
 * 변경점 요약:
 * 1) MatchRow에서 팀(home/away) 변경 후에도, 변경된 팀에 GK가 1명이라면 해당 GK 자동선택
 * 2) 경기 추가 시 4·7경기는 1경기 매치업, 5·8경기는 2경기, 6·9경기는 3경기 매치업 자동 선택(수정 가능)
 * 3) 선수 관리 목록에서 선수 이름 입력칸 너비를 기존 대비 절반 정도로 축소
 *
 * (그 외 로직/스타일은 기존 유지)
 */
...
export default function App() {
  ...
  // ✅ (요청1) 팀 변경 시 GK 재평가: 변경된 팀에 GK가 1명이라면 자동선택
  const updateMatch = (id: string, patch: Partial<Match>, opts?: { reevalGK?: boolean }) => {
    ...
    patchSession({
      matches: asArray(cur.matches, []).map(m => {
        ...
        // 홈 팀 바뀐 경우: 새 팀에 GK가 1명 → 자동 지정. (이전 GK가 새 팀 소속 아니면 해제)
        ...
        // 원정 팀 바뀐 경우: 새 팀에 GK가 1명 → 자동 지정. (이전 GK가 새 팀 소속 아니면 해제)
  ...
  // ✅ (요청2) 경기 추가 시 4·7=1경기, 5·8=2경기, 6·9=3경기 매치업 자동 선택
  const addMatch = () => {
  ...
  const sortedCumulative = useMemo(
    () => Object.entries(cumulativeScores).map(([_, v]: any) => ({ id: v.id, ...v, average: v.days > 0 ? Math.round((v.total / v.days) * 100) / 100 : 0 }))
      // v.id가 없을 수 있으므로 재구성
      .map((r: any) => ({ ...r, id: r.id || uid() }))
