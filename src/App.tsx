...
/**
 * 골딘 풋살 리그 · App.tsx (수정 반영 버전)
 * 변경점 요약:
 * - [요청#1] 팀 구성: 팀 카드에 '확정/수정' 토글 추가
 *   · 확정 시 선택된 팀원만 표시 (확정 후에도 '수정' 눌러 전체 보기로 전환 가능)
 *   · 세션(날짜)별로 rosterViewConfirmed 상태 저장
 * - [요청#2] 관리자 PIN 아래 페이지 탭 추가
 *   · 페이지1: 선수 관리
 *   · 페이지2: 일자별 경기 기록/결과/오늘의 개인 순위 (날짜 선택은 이 페이지에만 표시 및 적용)
 *   · 페이지3: 누적 순위
 */
...
type Session = {
  ...
  notes: string;
  /** [추가] 팀 카드 '확정' 상태 (확정 시 해당 팀 카드에는 선택된 선수만 표시) */
...
export default function App() {
  ...
  // [추가] 페이지 탭 (1: 선수 관리, 2: 일자별 기록/결과/개인 순위, 3: 누적 순위)
  ...
  // 경기 추가: 4·7=1경기, 5·8=2경기, 6·9=3경기 매치업 복제
  const addMatch = () => {
  ...
  return (
    <div className="wrap">
      ...
      <div className="tabs">
        ...
        <button className={page === 2 ? "tab active" : "tab"} onClick={() => setPage(2)}>경기 기록</button>
        <button className={page === 3 ? "tab active" : "tab"} onClick={() => setPage(3)}>누적 순위</button>
      </div>

      {/* [요청#2] 날짜 선택은 2번 페이지에서만 표시/적용 */}
      {page === 2 && (
        <div className="row">
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .tab { padding:8px 12px; border:1px solid #ddd; border-radius:999px; background:#f7f7f7; cursor:pointer; }
        ...
        /* 팀 득점 드롭다운 크기 */
        .score-input { width: 80px; height: 40px; border:1px solid #ccc; border-radius:8px; text-align:center; font-size:16px; }
