 ...
 * - 날짜 변경 깜빡임 방지(동기화 루프 차단)
 * - 날짜별 팀명 저장
 * - 선수 정렬: 필드 가나다 → GK 가나다(맨 아래)
 * - 기록창 선수 정렬 동일
 * - GK 점수 규칙:
 *    · 팀에 GK 1명: 팀 순위 보너스(4/2/1) 동일 적용
 *    · 팀에 GK 2명 이상: 그 팀 GK들 중 승수 1위 +4, 2위 +2
 * - CS 자동 집계(실점 0이면 지정 GK +1)
 * - Supabase Realtime 동기화
 * - ✅ GK 자동 설정(수정): 팀에 GK가 1명뿐이고 해당 칸이 “비어있을 때만” 기본값으로 채움
 *   → 사용자가 “선택 안 함”을 고르면 다시 채우지 않음
 */
