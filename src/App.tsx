...
type FifaStats = { PAC: number; SHO: number; PAS: number; DRI: number; DEF: number; PHY: number; };
type FifaData = { rating: number; stats: FifaStats; nationFlagUrl?: string; clubLogoUrl?: string; faceUrl?: string; };
const LS_FIFA = "goldin_fifa_v1";
function loadFifa(): Record<string, FifaData> {
  try { const raw = localStorage.getItem(LS_FIFA); return raw ? JSON.parse(raw) : {}; } catch { return {}; }
}
function saveFifa(map: Record<string, FifaData>) { try { localStorage.setItem(LS_FIFA, JSON.stringify(map)); } catch {} }
...
export default function App() {
  ...
  const [fifaMap, setFifaMap] = useState<Record<string, FifaData>>(loadFifa());
  useEffect(() => { saveFifa(fifaMap); }, [fifaMap]);
  // === FIFA editor state (선수 관리 페이지 전용) ===
  const [selectedFifaId, setSelectedFifaId] = useState<string>("");
  const selectedFifa: FifaData = selectedFifaId && fifaMap[selectedFifaId] ? fifaMap[selectedFifaId] : { rating: 50, stats: { PAC:50, SHO:50, PAS:50, DRI:50, DEF:50, PHY:50 } };

  function updateFifa(patch: Partial<FifaData>) {
    if (!selectedFifaId) return;
    setFifaMap(prev => ({ ...prev, [selectedFifaId]: { ...selectedFifa, ...patch, stats: { ...selectedFifa.stats, ...(patch.stats||{}) } } }));
  }

  // 업로드: Supabase가 있으면 Storage(fifa)로 업로드 → public URL 저장, 없으면 blob URL 임시 사용
  async function uploadImage(kind: "face"|"flag"|"club", file: File) {
    if (!selectedFifaId || !file) return;
    // try dynamic import of supabase client
    ...
    try {
      const sb = await (async () => {
        ...
        for (const c of candidates) {
          try { const m:any = await import(/* @vite-ignore */ c); const s = m?.supabase || m?.default || m; if (s) return s; } catch {}
        }
        return null;
      })();
      if (sb) {
        const ext = (file.name.split(".").pop() || "png").toLowerCase();
        const path = `${kind}s/${selectedFifaId}-${Date.now()}.${ext}`;
        const { error } = await sb.storage.from("fifa").upload(path, file, { upsert: true, cacheControl: "3600" });
        if (!error) {
          const { data } = await sb.storage.from("fifa").getPublicUrl(path);
          url = data?.publicUrl || null;
        }
      }
    } catch {}
    if (!url) url = URL.createObjectURL(file);
    if (kind === "face") updateFifa({ faceUrl: url });
    if (kind === "flag") updateFifa({ nationFlagUrl: url });
    if (kind === "club") updateFifa({ clubLogoUrl: url });
  }
  ...
  const analysisOptions = useMemo(() => {
    ...
    Object.entries(cumulativeScores).forEach(([pid, v]: any) => {
      if (v?.name) map.set(pid, v.name);
    });
    ...
    <div className="wrap">
      ...
      {page === 1 && (
        <section className="box">
          ...
          {/* === 능력치 카드 관리 (선수 관리에서만 수정 가능) === */}
          <div className="divider" style={{ height: 1, background: "#222", margin: "18px 0" }}></div>
          <h3>능력치 카드 관리</h3>
          <div className="fifa-admin" style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 16 }}>
            <div>
              <label className="fs-label">선수 선택</label>
              <select value={selectedFifaId || ""} onChange={e => setSelectedFifaId(e.target.value)}>
                <option value="">-- 선택 --</option>
                {players.map(p => (
                  <option key={p.id} value={p.id}>{p.name} ({p.pos}){p.active ? "" : " · 비활성"}</option>
                ))}
              </select>
              ...
              <div style={{ marginTop: 12 }}>
                {selectedFifaId && (
                  <FifaCard
                    name={players.find(p=>p.id===selectedFifaId)?.name || ""}
                    pos={(players.find(p=>p.id===selectedFifaId)?.pos === "GK" ? "GK" : "LM")}
                    rating={(fifaMap[selectedFifaId]?.rating) || 50}
                    stats={(fifaMap[selectedFifaId]?.stats) || { PAC:50, SHO:50, PAS:50, DRI:50, DEF:50, PHY:50 }}
                    nationFlagUrl={fifaMap[selectedFifaId]?.nationFlagUrl}
                    clubLogoUrl={fifaMap[selectedFifaId]?.clubLogoUrl}
                    faceUrl={fifaMap[selectedFifaId]?.faceUrl}
                    size={260}
                  />
                )}
              </div>
            ...
            {/* 우측 폼 */}
            <div className="fs-form fifa-section" >
              <div className="grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                <div>
                  <label>평점</label>
                  <input type="number" min={0} max={99}
                    value={selectedFifa.rating}
                    onChange={e => updateFifa({ rating: parseInt(e.target.value||"0")||0 })} />
                </div>
                <div><label>PAC</label><input type="number" min={0} max={99} value={selectedFifa.stats.PAC} onChange={e=>updateFifa({ stats: { ...selectedFifa.stats, PAC: parseInt(e.target.value||"0")||0 }})} /></div>
                <div><label>SHO</label><input type="number" min={0} max={99} value={selectedFifa.stats.SHO} onChange={e=>updateFifa({ stats: { ...selectedFifa.stats, SHO: parseInt(e.target.value||"0")||0 }})} /></div>
                <div><label>PAS</label><input type="number" min={0} max={99} value={selectedFifa.stats.PAS} onChange={e=>updateFifa({ stats: { ...selectedFifa.stats, PAS: parseInt(e.target.value||"0")||0 }})} /></div>
                <div><label>DRI</label><input type="number" min={0} max={99} value={selectedFifa.stats.DRI} onChange={e=>updateFifa({ stats: { ...selectedFifa.stats, DRI: parseInt(e.target.value||"0")||0 }})} /></div>
                <div><label>DEF</label><input type="number" min={0} max={99} value={selectedFifa.stats.DEF} onChange={e=>updateFifa({ stats: { ...selectedFifa.stats, DEF: parseInt(e.target.value||"0")||0 }})} /></div>
                <div><label>PHY</label><input type="number" min={0} max={99} value={selectedFifa.stats.PHY} onChange={e=>updateFifa({ stats: { ...selectedFifa.stats, PHY: parseInt(e.target.value||"0")||0 }})} /></div>
              </div>

              <div className="row2" style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8, marginTop: 10 }}>
                <input placeholder="국기 이미지 URL" value={selectedFifa.nationFlagUrl || ""} onChange={e=>updateFifa({ nationFlagUrl: e.target.value })} />
                <input type="file" accept="image/*" onChange={e => e.currentTarget.files?.[0] && uploadImage('flag', e.currentTarget.files[0])} />
              </div>
              <div className="row2" style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8 }}>
                <input placeholder="클럽 로고 URL" value={selectedFifa.clubLogoUrl || ""} onChange={e=>updateFifa({ clubLogoUrl: e.target.value })} />
                <input type="file" accept="image/*" onChange={e => e.currentTarget.files?.[0] && uploadImage('club', e.currentTarget.files[0])} />
              </div>
              <div className="row2" style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8 }}>
                <input placeholder="얼굴 URL" value={selectedFifa.faceUrl || ""} onChange={e=>updateFifa({ faceUrl: e.target.value })} />
                <input type="file" accept="image/*" onChange={e => e.currentTarget.files?.[0] && uploadImage('face', e.currentTarget.files[0])} />
              </div>
            </div>
          </div>
</section>
      ...
      <section className={`box ${page === 4 ? "" : "hidden"}`}>
        ...
        {selectedPlayerData && (
          <div className="analysis">
            ...
            <div className="charts" style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "flex-start" }}>
              <div style={{ flex: "0 0 296px" }}>
                <FifaCard
                  name={selectedPlayerData.name}
                  pos={(players.find(p=>p.id===selectedPlayerId)?.pos === "GK" ? "GK" : "LM")}
                  rating={(fifaMap[selectedPlayerId!]?.rating) || 50}
                  stats={(fifaMap[selectedPlayerId!]?.stats) || { PAC:50, SHO:50, PAS:50, DRI:50, DEF:50, PHY:50 }}
                  nationFlagUrl={fifaMap[selectedPlayerId!]?.nationFlagUrl}
                  clubLogoUrl={fifaMap[selectedPlayerId!]?.clubLogoUrl}
                  faceUrl={fifaMap[selectedPlayerId!]?.faceUrl}
                  size={260}
                />
              </div>
