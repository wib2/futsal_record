 ...
 *
 * ✨ 패치:
 * - "선수 명단"은 가나다 정렬, GK는 맨 아래(GK끼리는 가나다)로 표시
 *   · 선수 관리 리스트
 *   · 팀 구성 체크박스 리스트
 */
...
export default function App() {
  ...
  // ✅ 정렬: 가나다 + GK 맨 아래 (GK끼리는 가나다)
  const playersSorted = useMemo(() => {
    const field = [...players].filter(p => p.pos !== "GK").sort((a, b) => a.name.localeCompare(b.name, "ko"));
    const gks = [...players].filter(p => p.pos === "GK").sort((a, b) => a.name.localeCompare(b.name, "ko"));
    return [...field, ...gks];
  }, [players]);
  ...
  const addPlayer = (name: string) => {
    ...
    // 정렬은 렌더 때 playersSorted로 처리되므로 여기서는 추가만
      ...
      <div className="teams-grid">
        {TEAM_IDS.map(tid => (<div key={tid} className="team-card">
          ...
          <div className="list-scroll small">
            {activePlayersSorted.map(p => ( // ✅ 활성 + 정렬된 명단 사용
