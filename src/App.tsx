...
function FormationPreview({
  ...
  const markerSize = 14; // icon height; width is 1.25x inside UniformIcon

  // 분리: GK 1명만, 필드는 최대 4명
  const gkId = roster.find(id => players.find(p => p.id === id)?.pos === "GK") || null;
  const fields = roster.filter(id => players.find(p => p.id === id)?.pos !== "GK");
  const chosen = fields.slice(0, 4) as string[]; // 좌표는 필드 4개만

  // 필드 좌표
  const pts = FORMATION_POINTS[formation]; // 4개
  const coords = pts;
    ...
    <div className="formation-card">
      ...
      <svg viewBox="0 0 100 140" className="pitch">
        ...
        {coords.map((pt, i) => {
          ...
          const tx = pt.x - (markerSize * 0.625); // width center offset (size*1.25 / 2)
          const ty = pt.y - (markerSize / 2);
          return (
        ...
        {/* GK (정확히 1명일 때만) */}
        {gkId && (
          <g transform={`translate(${50 - (markerSize * 0.625)}, ${116 - (markerSize / 2)})`}>
            <UniformIcon fill={jerseyFill} size={markerSize} />
            <text
              x={markerSize * 0.625}
              y={markerSize / 2}
      ...
      {roster.length > chosen.length + (gkId ? 1 : 0) && (
        <div className="bench">벤치: {roster.filter(id => ![...chosen, gkId || ""].includes(id)).map(id => (players.find(p => p.id === id)?.name || "?")).join(", ")}</div>
      )}
    </div>
  );
}
