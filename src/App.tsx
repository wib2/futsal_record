 ...
 * - 최신 경기(큰 seq) 위로 정렬
 * - 모바일 UI 개선
 * - Supabase Realtime로 다기기 동기화
 *
 * ✨ 패치 요약
 * 1) ko-KR Collator로 정확한 가나다 정렬, GK는 명단 하단(GK끼리 가나다)
 * 2) 실시간 동기화 루프(날짜 깜박임) 차단:
 *    - 저장할 때 보정하지 않음
 *    - Cloud 수신값은 있는 그대로
 *    - 동일 상태면 setCloud 호출 안 함 (메타필드 무시 비교)
 *    - 초기 하이드레이션 완료 후에만 업로드
 *    - 날짜별 팀명 보정 useEffect에 강한 가드
 * 3) Realtime 키는 DB PK(int)에 맞춰 { id: 1 }
 */
...
type Standing = ReturnType<typeof computeStandings>;
...
/* ---------------- 비교 유틸: 메타필드(_rev, _clientId 등) 제외 ---------------- */
...
export default function App() {
  ...
  // ✅ 활성 선수만 정렬 적용
  const activePlayersSorted = useMemo(
    () => playersSorted.filter(p => p.active),
    [playersSorted]
  );
  ...
  const { value: cloud, setValue: setCloud, ready } =
    useRealtimeJsonState<typeof initialCloud>(initialCloud, { id: 1 as any }); // ✅ DB PK(int)에 맞춤

  // ✅ Cloud 수신 시: 있는 그대로 반영 + 하이드레이트 표시
  useEffect(() => {
  ...
  // ✅ 저장/동기화: 저장 시 보정하지 않고, 초기 하이드레이션 후 & 업무필드가 달라질 때만 setCloud
  useEffect(() => {
    const s = { players, teamNames: globalTeamNames, sessionsByDate, sessionDate };
    saveLocal(s);
    if (ready && hydrated) {
      if (!deepEqualState(cloud, s)) {
        setCloud(s);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [players, globalTeamNames, sessionsByDate, sessionDate, ready, hydrated]); // cloud를 의존성에서 제외
  ...
  useEffect(() => {
    ...
    setSessionsByDate(prev => {
      ...
      if (base.teamNames &&
          ...
          base.teamNames.C === useTN.C) {
        return prev;
      }

      const nextForKey = { ...base, teamNames: { A: useTN.A, B: useTN.B, C: useTN.C } };
      return { ...prev, [key]: nextForKey };
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sessionDate, globalTeamNames]); // sessionsByDate를 의존성에 넣지 않음
