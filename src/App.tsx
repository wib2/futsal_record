...
/**
 * 골딘 풋살 리그 · 기록/집계 (최종 App.tsx)
 * - 선수 명단 글로벌 유지
 * - 날짜: 일요일만 (viewer 모드도 날짜 선택 허용)
 * - 경기별 입력: 골/어시 (CS는 자동 계산만 반영)
 * - 수비상(+2), 팀 보너스(1위4/2위2/3위1)
 * - 오늘의 개인 순위 + 누적 순위(총점 기준, 평균은 보조)
 * - 자동 저장(LocalStorage) + 실시간 동기화(Supabase, 선택)
 * - 관리자 PIN / 보기 전용 링크
 */
...
export default function App(){
  ...
  // Save local + remote (debounced). We use our own rev timestamp; no need to change storage.ts
  useEffect(()=>{ const state:PersistShape={players,teamNames,sessionsByDate,sessionDate:ensureSunday(sessionDate)}; saveLocal(state);
    ...
    if(supaEnabled()){ if(saveTimerRef.current) window.clearTimeout(saveTimerRef.current);
      const nextRev=Date.now(); lastRevRef.current=nextRev;
      saveTimerRef.current=window.setTimeout(async()=>{ try{await remoteSave(state,nextRev);}catch{} }, SAVE_DEBOUNCE_MS) as unknown as number; }
  },[players,teamNames,sessionsByDate,sessionDate,readonly]);

  // Initial remote load + realtime subscribe (ignore stale or mine)
  ...
  function calcScores(session:Session){
    ...
    asArray(session.matches,[]).forEach(m=>{
      ...
      Object.entries(ms).forEach(([pid,s])=>{ const team=teamOf(pid); const b=out[pid]||{goals:0,assists:0,cleansheets:0}; const gg=asNumber((s as any).goals,0), aa=asNumber((s as any).assists,0);
        const cc=0; // CS는 자동 계산만 반영 (수동 입력 무시)
        out[pid]={goals:b.goals+gg,assists:b.assists+aa,cleansheets:b.cleansheets+cc,team}; });
  ...
  return (<div style={{maxWidth:1200,margin:"0 auto",padding:16,fontFamily:'system-ui, -apple-system, Segoe UI, Roboto, "Noto Sans KR", sans-serif'}}>
    ...
    <section style={box}><h3>경기 결과 (리그전)</h3>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
        <div style={{color:'#666',fontSize:14}}>승:3 / 무:1 / 패:0 · 각 경기의 <b>기록</b>에서 G/A 입력 (CS는 자동)</div>
    ...
    <section style={box}><h3>누적 순위 (모든 날짜)</h3>
      <div style={{overflowX:'auto'}}><table style={table}>
        ...
        <tbody>{sortedCumulative.map((r:any,idx:number)=>(<tr key={r.id}><td>{idx+1}</td><td>{r.name}</td><td>{r.days}</td><td>{r.goals}</td><td>{r.assists}</td><td>{r.cleansheets}</td><td>{r.def}</td><td>{r.teamBonus}</td><td style={{fontWeight:700}}>{r.total}</td><td>{r.average}</td></tr>))}</tbody>
      </table></div>
    </section>
      ...
      {[...(rosterA||[]),...(rosterB||[])].map(pid=>{ const v=values[pid]||{goals:0,assists:0,cleansheets:0}; return (
        <div key={pid} style={{border:"1px solid #f0f0f0",borderRadius:8,padding:8}}>
          ...
          <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
            ...
            <label title="자동 계산">CS</label><input type="number" value={v.cleansheets} onChange={()=>{}} style={{width:60}} disabled={true}/>
          </div>
        </div> ); })}
    </div></div>)}
  </div>);
