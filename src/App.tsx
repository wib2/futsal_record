...
/**
 * 골딘 풋살 리그 · App.tsx (최신 완성본)
 *
 * 포함된 기능
 * - 날짜 변경 깜빡임 방지(동기화 루프 차단: deepEqual + debounce + syncLock)
 * - 날짜별 팀명 개별 설정(없으면 직전/글로벌 1회 복사)
 * - 선수 정렬: 필드 가나다 → GK 가나다(맨 아래)
 * - 기록창 선수 정렬 동일(필드→GK)
 * - GK 승수 보너스:
 *    · 팀에 GK 1명: 해당 GK도 팀 순위 보너스(4/2/1)
 *    · 팀에 GK 2명 이상: 그 팀 GK들 중 그날 승수 1위 +4, 2위 +2 (필드 선수는 팀 순위 보너스)
 * - CS: 실점 0이면 지정된 GK에게 +1
 * - 수비상(defAwards): 팀별 지정 선수 +2
...
const LS_KEY = "goldin_futsal_app_main_v13";
...
export default function App() {
  ...
  // Realtime (클라우드 상태) — id는 Supabase 테이블의 정수 PK와 일치해야 함
    ...
    Object.keys(out).forEach(pid => {
      ...
      if (team !== "-") {
        if (isGK) {
          ...
          if (gks.length <= 1) {
            // GK 1명: 팀 순위 보너스 동일 적용
            teamBonus = teamBonusByTeam[team] || 0;
          } else {
            // GK 2명 이상: 그 팀 GK들 중 승수 순으로 1위+4, 2위+2
