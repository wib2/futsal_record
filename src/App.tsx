      ...
      if (team !== "-") {
        if (isGK) {
          ...
          if (gks.length <= 1) {
            ...
            const wa = gkWins[pid] || 0;
            const ranked = [...gks].sort((a, b) => {
              const da = (gkWins[a] || 0), db = (gkWins[b] || 0);
    ...
    <div className="wrap">
      ...
      {page === 1 && (
        <section className="box">
          ...
          <div className="list-scroll players-admin-grid">
            {playersSorted.map(p => (
              <div key={p.id} className="row player-admin-item">
                <input
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        /* [EDIT] 선수관리: 데스크탑 3열 그리드 */
        .players-admin-grid { display: grid; grid-template-columns: 1fr; gap: 8px; }
        .player-admin-item { border:1px solid var(--line); border-radius:10px; padding:8px; background:#0B0C10; }
        @media (min-width: 1024px) {
          .players-admin-grid { grid-template-columns: repeat(3, 1fr); }
        }
        ...
        .ranking-section h4 { margin: 8px 0 10px; }

        /* [EDIT] 랭킹 카드 컨테이너: 데스크탑에서 5개가 한 줄에 */
        .rank-cards { display:flex; flex-wrap: wrap; gap: 8px; }
        .rank-card {
          display:flex; align-items:center;
          background:#1c1c1f; border-radius:10px; padding:8px 10px; 
          color:var(--text); border:1px solid #24262c;
          gap:6px;
          flex: 1 1 calc(50% - 8px); /* 모바일/좁은 화면에서 2열 */
        }
        @media (min-width: 1024px) {
          .rank-card { flex: 0 0 calc(20% - 8px); } /* 5개 = 한 줄 */
        }
        .rank-card .rank-badge { font-weight:800; color:var(--gold); }
        .rank-card .player-name { flex:1 1 auto; margin:0 6px; overflow:hidden; white-space:nowrap; text-overflow:ellipsis; }
        /* [EDIT] 숫자 색을 골딘 글자색으로 통일 */
        .rank-card .player-score { font-weight:800; color:var(--gold); }
