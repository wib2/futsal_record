...
const FORMATION_POINTS: Record<FormationKey, { x: number; y: number; label: string }[]> = {
  ...
  "2-2-1": [
    { x: 50, y: 92, label: "GK" },
    { x: 30, y: 70, label: "DF" }, { x: 70, y: 70, label: "DF" },
    { x: 30, y: 40, label: "MF" }, { x: 70, y: 40, label: "MF" },
    { x: 50, y: 20, label: "FW" }
  ],
...
function FormationPreview({
  ...
  // 선수/포지션 분리 (GK는 정확히 1명일 때만 표시)
  const gks = roster.filter(id => (players.find(p => p.id === id)?.pos) === "GK");
  const fielders = roster.filter(id => (players.find(p => p.id === id)?.pos) !== "GK");
  const includeGK: string | null = (gks.length === 1) ? gks[0] : null;

  // 필드 인원수 기준 포메이션 자동 선택
  const fCount = fielders.length;
  const formationKey: FormationKey = (fCount <= 3) ? "1-2" : (fCount === 4 ? "1-2-1" : "2-2-1");
  const pts = FORMATION_POINTS[formationKey];
  const coords = includeGK ? pts : pts.slice(1); // GK 없으면 GK 좌표 제외

  const jerseyFill =
    color === "red" ? "var(--jersey-red)" :
    color === "yellow" ? "var(--jersey-yellow)" : "var(--jersey-white)";

  const placed: { pid: string, x: number, y: number }[] = [];
  if (includeGK) {
    placed.push({ pid: includeGK, x: coords[0].x, y: coords[0].y });
  }
  const fieldPts = includeGK ? coords.slice(1) : coords;
  const usedFielders = fielders.slice(0, fieldPts.length);
  usedFielders.forEach((pid, i) => placed.push({ pid, x: fieldPts[i].x, y: fieldPts[i].y }));
  return (
    <div className="formation-card">
      ...
      <svg viewBox="0 0 100 140" className="pitch">
        ...
        {/* players */}
        {placed.map((it, i) => {
          const name = players.find(p => p.id === it.pid)?.name || "";
          const x = it.x, y = it.y;
          const markerSize = 20; // 기존 대비 ~2배
          return (
            <g key={i} transform={`translate(${x}, ${y})`}>
              <UniformIcon fill={jerseyFill} size={markerSize} />
              <text
                x={markerSize/2}
                y={markerSize/2 + 1}
                textAnchor="middle"
