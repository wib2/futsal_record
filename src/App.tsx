  ...
  return (
    <div className="formation-card">
      ...
      {(() => { const used = new Set([...(chosenFields||[]), ...(singleGK ? [singleGK] : [])]); return roster.filter(id => !used.has(id)).length > 0; })() && (
        <div className="bench">{(() => {
  // Build from fields + gkIds to avoid relying on outer "roster" symbol
  const baseIds = ([] as any[]).concat(fields || [], gkIds || []);
  const used = new Set([...(chosenFields||[]), ...(singleGK ? [singleGK] : [])]);
  const benchIds = baseIds.filter((id:any) => !used.has(id));
  const isAllGK = benchIds.length > 0 && benchIds.every((id:any) => {
    const pos = players.find(p => p.id === id)?.pos;
    return pos === "골키퍼" || pos === "GK";
  });
  const label = isAllGK ? "GK" : "벤치";
  const names = benchIds.map((id:any) => (players.find(p => p.id === id)?.name || "?")).join(", ");
          ...
          <div className="team-col">
            ...
            <div className="players-grid">
              {asArray(rosterA_sorted, []).map(pid => {
                ...
                return (
                  <div key={pid} className="player-card" style={{opacity: p.active ? 1 : 0.55}}>
...
const fieldCount = (chosenFields?.length || 0);
