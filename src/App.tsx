...
export default function App() {
  ...
  // ===== FIFA 공유 데이터 (Supabase) =====
  type FifaStats = { PAC:number; SHO:number; PAS:number; DRI:number; DEF:number; PHY:number };
  type FifaRow = { player_id:string; rating:number; stats:FifaStats; nation_flag_url?:string|null; club_logo_url?:string|null; face_url?:string|null; updated_at?:string };

  const [fifaMap, setFifaMap] = useState<Record<string, FifaRow>>({});
  ...
  useEffect(() => { (async () => {
    ...
    if (!sb) return;
    try {
      const { data, error } = await sb.from("fifa_cards").select("*");
      if (!error && data) {
        const map: Record<string, FifaRow> = {};
        data.forEach((r: any) => { map[r.player_id] = r as FifaRow; });
        setFifaMap(map);
      }
    } catch (_) {}
  })(); }, []);

  const upsertFifaRows = async (rows: any[]) => {
    const sb = await loadSupabaseClient();
    if (!sb || !rows?.length) return;
    try {
      await sb.from("fifa_cards").upsert(rows, { onConflict: "player_id" });
      setFifaMap(prev => {
        const nx = { ...prev };
        rows.forEach(r => { nx[r.player_id] = { ...(prev[r.player_id]||{}), ...r }; });
        return nx;
      });
    } catch (_) {}
  };

  const onPlayersChangeFromFifa = async (nextPlayers: any[]) => {
    // diff fifa changes and persist
    const prevMap: Record<string, string> = Object.fromEntries(players.map((p: any) => [p.id, JSON.stringify(p.fifa || {})]));
    const changed = nextPlayers.filter((p: any) => JSON.stringify(p.fifa || {}) != (prevMap[p.id] || "{}"));
    setPlayers(nextPlayers);
    const rows = changed.filter((p: any) => p.fifa).map((p: any) => ({
      player_id: p.id,
      rating: p.fifa.rating,
      stats: p.fifa.stats,
      nation_flag_url: p.fifa.nationFlagUrl || null,
      club_logo_url: p.fifa.clubLogoUrl || null,
      face_url: p.fifa.faceUrl || null,
    }));
    if (rows.length) await upsertFifaRows(rows);
  };
  ...
  const addMatch = () => {
    ...
    const pairBySeq: [TeamId, TeamId][] = [["A","B"],["B","C"],["C","A"]];
    const [home, away] = pairBySeq[(nextSeq - 1) % 3];
  ...
  const selectedPlayerObj = selectedPlayerId ? players.find(p => p.id === selectedPlayerId) || null : null;
  const selectedPlayerData = selectedPlayerId
    ? (sortedCumulative.find(p => p.id === selectedPlayerId) || { id: selectedPlayerId, name: selectedPlayerObj?.name || "", teamName: "", goals: 0, assists: 0, def: 0, cleansheets: 0, teamBonus: 0, total: 0 })
    : null;

  // 공유용 FIFA 카드 데이터 (Supabase 테이블 우선, 없으면 로컬 p.fifa)
  const selectedFifaRow = selectedPlayerId ? (fifaMap[selectedPlayerId] || (selectedPlayerObj?.fifa ? {
    player_id: selectedPlayerObj.id,
    rating: selectedPlayerObj.fifa.rating,
    stats: selectedPlayerObj.fifa.stats,
    nation_flag_url: selectedPlayerObj.fifa.nationFlagUrl,
    club_logo_url: selectedPlayerObj.fifa.clubLogoUrl,
    face_url: selectedPlayerObj.fifa.faceUrl,
  } : null)) : null;const globalMaxValue = useMemo(() => {
  ...
  return (
    <div className="wrap">
      ...
      {page === 1 && (<>
        ...
        {authed && (
          <section className="box" style={{ marginTop: 12 }}>
            <h3>능력치 카드 관리</h3>
            <FifaSection players={players} onChange={onPlayersChangeFromFifa} isAdmin={true} />
          </section>
        )}
</>)}
              ...
              <div style={{ flex: "0 0 260px", minWidth: 260 }}>
                {selectedPlayerObj && (
                  <FifaCard
                    ...
                    pos={(selectedPlayerObj.pos || "필드").toUpperCase()}
                    rating={selectedFifaRow?.rating ?? 50}
                    stats={selectedFifaRow?.stats ?? { PAC:50, SHO:50, PAS:50, DRI:50, DEF:50, PHY:50 }}
                    nationFlagUrl={selectedFifaRow?.nation_flag_url || undefined}
                    clubLogoUrl={selectedFifaRow?.club_logo_url || undefined}
                    faceUrl={selectedFifaRow?.face_url || undefined}
                    size={260}
                  />
                )}
              </div>
