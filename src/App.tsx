...
export default function App() {
  ...
  const [page, setPage] = useState<1 | 2 | 3 | 4 | 5>(2); // ★ 탭 확장
          ...
          <div className="ranking-grid">
            {[
              ...
              <div className="ranking-section" key={cat.key}>
                <h4>{cat.title}</h4>
                {top5Ranking(cat.key as any).map((p, i) => (
                  <div key={p.id} className={`rank-card rank-${i+1}`}>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .rank-card {
          ...
          background: #1c1c1f; border-radius: 10px; padding: 10px 12px; margin-bottom: 6px;
          color: var(--text); border: 1px solid #24262c;
        }
        .rank-card .rank-badge { font-weight: 800; color: var(--gold); margin-right: 8px; }
        .rank-card .player-name { flex: 1; margin-left: 8px; }
