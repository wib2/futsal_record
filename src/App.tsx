    ...
    <div className="formation-card">
      ...
      {(() => { const used = new Set([...(chosenFields||[]), ...(singleGK ? [singleGK] : [])]); return roster.filter(id => !used.has(id)).length > 0; })() && (
        <div className="bench">{(() => {
          ...
          const benchIds = roster.filter(id => !used.has(id));
          const isAllGK = benchIds.length > 0 && benchIds.every(id => (players.find(p => p.id === id)?.pos === "골키퍼"));
          const label = isAllGK ? "GK" : "벤치";
          return <><span>{label}: </span>{benchIds.map(id => (players.find(p => p.id === id)?.name || "?")).join(", ")}</>;
        })()}</div>
      )}
    </div>
  );
...
export default function App() {
  ...
  // PWA manifest override to match existing icon filenames in /public
  useEffect(() => {
    try {
      // Remove old manifest link(s)
      Array.from(document.querySelectorAll('link[rel="manifest"]')).forEach(el => el.parentElement?.removeChild(el));
      const manifest = {
        name: "Goldin Futsal",
        short_name: "Goldin",
        start_url: "/",
