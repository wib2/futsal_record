...
/**
 * 골딘 풋살 리그 · App.tsx (DARK THEME · GOLDIN PALETTE)
 * - 다크 테마로 전체 리스킨 (엠블럼 골드/차콜/화이트)
 * - 기존 기능 그대로 유지: 페이지 탭, 날짜(2번 페이지만), 팀 구성 확정/수정
 */
...
function normalizeLoaded(data: any): PersistShape {
  ...
  if (data?.sessionsByDate && typeof data.sessionsByDate === "object") {
    for (const [k, v] of Object.entries<any>(data.sessionsByDate)) {
      ...
      sessionsByDate[ensureSunday(k)] = { rosters, matches, matchStats, defAwards, teamNames, notes, rosterViewConfirmed };
    }
  } else {
...
export default function App() {
  ...
  const cumulativeScores = useMemo(() => {
    ...
    Object.entries(sessionsByDate).forEach(([dateKey, s]) => {
      // dateKey는 세션키(일요일 ISO) — order를 위해 키와 함께 보관
  ...
  /* ------------ 여기부터 추가: Top5 랭킹 보드 / 선수별 분석 ------------ */

  // 5위 동점자 전원 노출 Top5 유틸
  function top5Ranking(type: "goals" | "assists" | "def" | "cleansheets" | "total") {
    const sorted = [...sortedCumulative].sort((a, b) => (b[type] || 0) - (a[type] || 0));
    if (sorted.length <= 5) return sorted;
    const fifthScore = sorted[4][type] || 0;
    return sorted.filter(p => (p[type] || 0) >= fifthScore);
  }
  ...
  // 선수별 스파크라인(날짜별 총점) — 세션 키(일요일 ISO) 기준 정렬
  const sparklineData = useMemo(() => {
    ...
    return entries.map(([dateKey, s]) => {
      ...
      return { date: dateKey.slice(5), total: row.total }; // MM-DD만 표시
    });
    ...
    <div className="wrap">
      ...
      {page === 1 && (
        <section className="box">
          ...
          <div className="list-scroll players-grid-admin">
            {playersSorted.map(p => (
        ...
        <>
          ...
          <section className="box">
            ...
            <div className="row" style={{ gap: 24, flexWrap: "wrap" }}>
              <div className="mini-table">
                <h4>득점왕</h4>
                {top5Ranking("goals").map((p, i) => (
                  <div key={p.id}>{i + 1}. {p.name} — {p.goals}</div>
                ))}
              </div>
              <div className="mini-table">
                <h4>도움왕</h4>
                {top5Ranking("assists").map((p, i) => (
                  <div key={p.id}>{i + 1}. {p.name} — {p.assists}</div>
                ))}
              </div>
              <div className="mini-table">
                <h4>수비왕</h4>
                {top5Ranking("def").map((p, i) => (
                  <div key={p.id}>{i + 1}. {p.name} — {p.def}</div>
                ))}
              </div>
              <div className="mini-table">
                <h4>클린시트왕</h4>
                {top5Ranking("cleansheets").map((p, i) => (
                  <div key={p.id}>{i + 1}. {p.name} — {p.cleansheets}</div>
                ))}
              </div>
            </div>
          </section>
  ...
  --goldin-text:#F5C542;
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .score-input { width: 80px; height: 40px; border:1px solid var(--line); border-radius:8px; text-align:center; font-size:16px; background:#0D0F14; color: var(--text); }
        ...
        .player-card { border:1px solid var(--line); border-radius:10px; padding:8px; background:#0B0C10; display:flex; align-items:center; gap:10px; flex-wrap: wrap; }
        .player-name { flex:1 1 220px; min-width: 160px; font-weight:600; color: var(--text); }
        .pos { color:#9aa0aa; font-size:12px; }

        .stat { display:inline-flex; align-items:center; gap:6px; }
        .stat-label { font-weight:600; color:#cfcfcf; }
        .stat-input { width:72px; height:40px; padding:6px 8px; border:1px solid var(--line); border-radius:8px; text-align:center; font-size:16px; background:#0D0F14; color: var(--text); }
        .stat-input.select { width:72px; height:40px; }
        ...
        .mini-table h4 { margin: 6px 0 8px; }
        .analysis h4 { margin: 0 0 8px; }

        @media (max-width: 900px) { .scoreline { grid-template-columns: 1fr 68px 20px 68px 1fr; } .score-input { width: 68px; } }
        ...
        @media (max-width: 360px) { .scoreline { grid-template-columns: minmax(0,1fr) 50px 12px 50px minmax(0,1fr); } .stat-input { width: 58px; } .score-input { width: 50px; } }
...
.rank-card .rank-badge{ color: var(--goldin-text); }

.ranking-grid{ display:grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap:12px; }

@media (min-width: 1200px){
  .ranking-grid { grid-template-columns: repeat(5, 1fr); }
  .rank-card { padding: 6px 8px; }
  .rank-card .player-name { margin-left: 4px; font-size: 14px; }
  .rank-card .player-score { font-size: 14px; }
}

.players-grid-admin{ display:grid; grid-template-columns: 1fr; gap:8px; }
@media (min-width: 1200px){ .players-grid-admin{ grid-template-columns: repeat(3, 1fr); } }
`}</style>
    </div>
  );
}
