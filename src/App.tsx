...
// === PERSIST WRITE-GATE (minimal, no side effects) ===
let __persistHydrated = false;
let __persistLastNonEmpty: any = null;

function __isEmptyPersistState(s: any): boolean {
  try {
...
async function __rawSavePersistToSupabase(state: AppPersist): Promise<void> {
  try {
...
useEffect(() => { (async () => {
  try {
    ...
    __persistHydrated = true;
    if (server && !__isEmptyPersistState(server)) __persistLastNonEmpty = server;
    ...
    <div className="wrap">
      ...
      <div className="panel">
        ...
        {authed ? <>
          ...
          <button onClick={copyViewerLink}>보기 전용 링크 복사</button> <button onClick={exportData}>데이터 내보내기</button>
        </> : <>
      ...
      {page === 1 && (
        <section className="box">
          <h3>선수 관리</h3>
          {/* FIFA 카드 섹션 (관리자 전용 편집, 보기 전용은 읽기만) */}
          <FifaSection players={playersSorted.map(p=>({id:String(p.id), name:(typeof name === "function" ? name(p.id) : (p?.name ?? (p?.nm ?? String(p.id))))}))} readonly={readonly} />
...
<AddPlayer onAdd={(nm) => addPlayer(nm)} disabled={readonly} />
          ...
          <section className="box">
            ...
            <div className="teams-grid">
              {TEAM_IDS.map(tid => {
                ...
                const fieldCount = currentRoster.filter(pid => (players.find(p => p.id === pid)?.pos) !== "GK").length;
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .rank-card .rank-badge { font-weight:800; color:var(--emblem-word); font-size:14px; line-height:1; }
...
// === WRITE-GATE wrapper for savePersistToSupabase ===
async function savePersistToSupabase(state: any): Promise<void> {
  // Block any write before initial hydration unless it's a NON-empty state
  if (!__persistHydrated) {
    if (__isEmptyPersistState(state)) return;
    __persistHydrated = true;
  }
  // Never allow empty state to overwrite server
  if (__isEmptyPersistState(state)) return;
  __persistLastNonEmpty = state;
  return __rawSavePersistToSupabase(state);
}
// === END wrapper ===