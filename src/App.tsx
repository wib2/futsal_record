 ...
 *
 * ✨ 패치:
 * - "선수 명단"은 가나다 정렬(ko-KR Collator), GK는 맨 아래(GK끼리는 가나다)로 표시
 *   · 선수 관리 리스트
 *   · 팀 구성 체크박스 리스트
 */
...
export default function App() {
  ...
  const updateTeamName = (tid: TeamId, nm: string) => patchSession({ teamNames: { ...(cur.teamNames || effectiveTeamNames), [tid]: nm } });
  ...
  const sortedDaily = useMemo(() => Object.entries(dailyScores).map(([pid, v]: any) => ({ id: pid, ...v })).sort((a, b) => b.total - a.total || compareKo(a.name, b.name)), [dailyScores, compareKo]);
  ...
  const sortedCumulative = useMemo(() => Object.entries(cumulativeScores).map(([pid, v]: any) => ({ id: pid, ...v, average: v.days > 0 ? Math.round((v.total / v.days) * 100) / 100 : 0 })).sort((a, b) => b.total - a.total || compareKo(a.name, b.name)), [cumulativeScores, compareKo]);
