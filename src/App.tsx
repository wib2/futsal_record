...
function FormationPreview({
  ...
  /* 5명: GK 1 + 필드 4 자동 선정 */
  const gk = roster.find(id => players.find(p => p.id === id)?.pos === "GK") || null;
  const fields = roster.filter(id => players.find(p => p.id === id)?.pos !== "GK");
  const chosen = [gk, ...fields].filter(Boolean).slice(0, 5) as string[];
        ...
        {coords.map((pt, i) => {
          ...
          return (
            <g key={i} transform={`translate(${pt.x}, ${pt.y})`} textAnchor="middle">
              <UniformIcon fill={jerseyFill} size={16} stroke="var(--jersey-stroke)" />
              <text className="player-initials" dominantBaseline="middle" dy="0.3em">{initialsTxt}</text>
              <text className="player-label" y="14" dominantBaseline="hanging">{pid ? name : "빈 자리"}</text>
            </g>
          );
        })}
      </svg>
