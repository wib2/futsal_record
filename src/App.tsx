/* App.tsx — 골딘 기록앱 (players 3열 압축 & 랭킹보드 5개/행 맞춤) */
    ...
    <div className="wrap">
      ...
      {page === 1 && (
        <section className="box">
          ...
          {/* [EDIT] 데스크탑 3열 + 내부 요소 폭 압축 (가로 스크롤 제거) */}
          ...
          <div className="ranking-grid">
            {[
              ...
              <div className="ranking-section" key={cat.key}>
                <h4>{cat.title}</h4>
                {/* [EDIT] 5개가 한 줄에 들어오도록 카드 폭/간격/내부 여백 축소 */}
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        /* [EDIT] 선수관리: 3열 고정 + 내부 컴팩트 */
        .players-admin-grid { display: grid; grid-template-columns: 1fr; gap: 8px; }
        @media (min-width: 1024px) {
          .players-admin-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); }
        }
        .player-admin-item {
          display:grid; grid-template-columns: 1fr 84px 78px; gap:6px;
          border:1px solid var(--line); border-radius:10px; padding:6px; background:#0B0C10;
        }
        /* 이름 칸이 줄어들 수 있도록 min-width 제거 */
        ...
        .ranking-section h4 { margin: 6px 0 8px; }

        /* [EDIT] 5개가 한 줄에 들어오도록 카드 폭/간격/내부 여백 최적화 */
        .rank-cards { display:flex; flex-wrap: wrap; gap: 6px; }
        .rank-card {
          display:flex; align-items:center;
          background:#1c1c1f; border-radius:10px; padding:6px 8px;
          color:var(--text); border:1px solid #24262c;
          gap:4px;
          flex: 1 1 calc(50% - 6px); /* 모바일 2열 */
        }
        @media (min-width: 1024px) {
          .rank-card { flex: 0 0 calc(20% - 6px); } /* 데스크톱 정확히 5개/행 */
        }
        /* [EDIT] 순위 숫자/점수 색상 골딘 골드 */
        .rank-card .rank-badge { font-weight:800; color:var(--gold); font-size:14px; line-height:1; }
        .rank-card .player-name {
          flex:1 1 auto; margin:0 4px; overflow:hidden; white-space:nowrap; text-overflow:ellipsis;
          font-size:14px;
        }
        /* [EDIT] 이름-점수 간격 축소 + 점수 골드 */
