...
/**
 * 골딘 풋살 리그 · 기록/집계 (모바일 UI + 경기별 G/A만 + 고정 PIN + 날짜별 팀명)
 * - PIN: 전역 고정값 8347 (어느 기기든 동일) — 설정 UI 제거, 잠금해제만
 * - 팀명: 날짜별로 따로 저장/표시 (세션마다 teamNames 갖고, 없으면 이전/글로벌에서 복사)
 * - 경기별 입력: 골/어시만 (CS 입력란 제거, CS는 자동 계산만 반영)
 * - 최신 추가 경기(큰 seq)가 목록 맨 위
 * - 모바일 테이블/보더 겹침 개선
 */
...
type Session = {
  ...
  teamNames?: Record<TeamId,string>; // ✅ 날짜별 팀명
  notes: string;
};
type PersistShape = {
  players: Player[];
  teamNames: Record<TeamId,string>; // 구버전 호환(글로벌)
...
const toISO = (d: Date) => { const y=d.getFullYear(), m=String(d.getMonth()+1).padStart(2,"0"), da=String(d.getDate()).padStart(2,"0"); return `${y}-${m}-${da}`; };
const ensureSunday = (iso: string) => { const d = new Date(iso); if (Number.isNaN(d.getTime())) { const t=new Date(); const add=(7-t.getDay())%7; return toISO(new Date(t.getTime()+add*DAY_MS)); } const add=(7-d.getDay())%7; return toISO(new Date(d.getTime()+add*DAY_MS)); };
...
function computeStandings(matchesInput:Match[]|null|undefined): StandingRow[] {
  ...
  for(const m of matches){ const HG=asNumber(m.hg,0), AG=asNumber(m.ag,0);
...
function normalizeLoaded(data:any): PersistShape {
  ...
  if(data?.sessionsByDate && typeof data.sessionsByDate==="object"){
    for(const [k,v] of Object.entries<any>(data.sessionsByDate)){
      ...
      Object.keys(rawMS).forEach(mid=>{ const row = rawMS[mid]||{}; const safe: MatchStats = {} as any; Object.keys(row).forEach(pid=>{ const v=row[pid]||{}; safe[pid] = { goals:asNumber(v.goals,0), assists:asNumber(v.assists,0), cleansheets:asNumber(v.cleansheets,0) }; }); matchStats[mid]=safe; });
...
function loadState(): PersistShape|null { try { const raw=localStorage.getItem(LS_KEY); return raw? normalizeLoaded(JSON.parse(raw)): null; } catch { return null; } }
function saveState(s: PersistShape){ try { localStorage.setItem(LS_KEY, JSON.stringify(s)); } catch {} }

const isEmptyRosters = (r:Record<TeamId,string[]>) => TEAM_IDS.every(t=> asArray(r?.[t],[]).length===0);
...
// ---------------- Components ----------------
function MatchRow({ m, readonly, updateMatch, deleteMatch, rosterA, rosterB, players, values, onChange, teamNames }:{
...
export default function App(){
  ...
  const init = loadState() ?? fallback;

  const [players, setPlayers] = useState<Player[]>(init.players);
  const [globalTeamNames, setGlobalTeamNames] = useState<Record<TeamId,string>>(init.teamNames);
  const [sessionDate, setSessionDate] = useState<string>(init.sessionDate);
  const [sessionsByDate, setSessionsByDate] = useState<Record<string,Session>>(init.sessionsByDate);
  ...
  const [pinInputLocal,setPinInputLocal] = useState("");
  ...
  // save on change
  useEffect(()=>{
    saveState({ players, teamNames: globalTeamNames, sessionsByDate, sessionDate: ensureSunday(sessionDate) });
  },[players, globalTeamNames, sessionsByDate, sessionDate]);

  // helpers to patch current session
  ...
  const toggleRoster = (tid:TeamId, pid:string)=> patchSession({ rosters:(()=>{ const r={...(cur.rosters||{A:[],B:[],C:[]})}; const list=asArray(r[tid],[]); r[tid]= list.includes(pid)? list.filter(id=>id!==pid) : [...list,pid]; return r; })() });
  const addMatch = ()=> {
  ...
  // change date (viewer도 허용)
  const changeDate = (v:string)=>{
    const sunday = ensureSunday(v);
    setSessionDate(sunday);
    if (readonly) return;
    setSessionsByDate(prev=>{
      let sess = prev[sunday] ?? emptySession();
      if (isEmptyRosters(sess.rosters)){
        const donor = findDonorRosters(prev, sunday);
        if (donor) sess = {...sess, rosters: donor};
      }
      if (!sess.teamNames) sess = {...sess, teamNames: effectiveTeamNames};
      return { ...prev, [sunday]: sess };
    });
  };
  ...
  // PIN / Viewer UI  (고정 PIN: 8347)
  async function unlock(){
    const input = (document.getElementById("pin-input") as HTMLInputElement | null)?.value || "";
    if(!input) return alert("PIN을 입력하세요");
    const h = await sha256Hex(input);
    if (h===FIXED_PIN_HASH || input==="8347") {
      sessionStorage.setItem(SS_PIN_AUTHED, "1");
      setAuthed(true);
      (document.getElementById("pin-input") as HTMLInputElement | null)?.blur();
    } else { alert("PIN 불일치"); }
  }
  ...
  return (<div className="wrap">
    ...
    <div className="panel">
      ...
      {authed ? <>
        ...
        <input id="pin-input" type="password" placeholder="PIN 입력(8347)"/>
    ...
    <div className="row">
      ...
      <input type="date" value={sessionDate} onChange={e=>changeDate(e.target.value)} />
      ...
      <div className="teams-grid">
        {TEAM_IDS.map(tid=>(<div key={tid} className="team-card">
          ...
          <div className="list-scroll small">
            {players.filter(p=>p.active).map(p=>(<label key={p.id} className="checkline">
    ...
    <style>{`
      ...
      .scoreline { display:grid; grid-template-columns: 1fr 60px 20px 60px 1fr; gap:6px; align-items:center; }
      ...
      .players-grid { display:grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap:8px; }
      .player-card { border:1px solid #eee; border-radius:10px; padding:8px; background:#fff; display:grid; grid-template-columns: auto 1fr 40px auto 40px; gap:6px; align-items:center; }
      .player-name { grid-column: 1 / -1; font-weight:600; }
      .pos { color:#999; font-size:12px; }

      input, select, textarea, button { padding: 6px 8px; border: 1px solid #ccc; border-radius: 8px; background:#fff; }
