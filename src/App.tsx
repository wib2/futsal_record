  ...
  return (
    <div className="formation-card">
      ...
      {(() => { const used = new Set([...(chosenFields||[]), ...(singleGK ? [singleGK] : [])]); return roster.filter(id => !used.has(id)).length > 0; })() && (
        <div className="bench">{(() => { const used2 = new Set([...(chosenFields||[]), ...(singleGK?[singleGK]:[])]); const benchIds = roster.filter(id => !used2.has(id)); const gkCount = benchIds.filter(id => (players.find(p=>p.id===id)?.pos) === "GK").length; const label = (!singleGK && gkCount > 0) ? "GK: " : "벤치: "; return label; })()}{(() => { const used = new Set([...(chosenFields||[]), ...(singleGK ? [singleGK] : [])]); return roster.filter(id => !used.has(id)).map(id => (players.find(p => p.id === id)?.name || "?")).join(", "); })()}</div>
      )}
    </div>
  );
}
...
export default function App() {
  ...
  // Admin URL: always hydrate from server (latest wins)
  // Viewer URL or default: hydrate only if local is blank (avoid clobbering local offline edits)
  useEffect(() => { (async () => {
    try {
      const server = await loadPersistFromSupabase();
      if (server && !isBlankPersist(server)) {
        const localNow = { players, teamNames: globalTeamNames, sessionsByDate, sessionDate: ensureSunday(sessionDate) };
        if (adminMode || isBlankPersist(localNow)) {
    ...
    <div className="wrap">
      ...
      {page === 2 && (
        ...
        <section className="box">
          ...
          <div className="teams-grid">
            {TEAM_IDS.map(tid => {
              const rosterIds = asArray(cur.rosters[tid], []);
              const options = rosterIds.map(pid => ({ id: pid, name: players.find(p => p.id === pid)?.name || "?" }));
              const curSel = (session.defAwards?.[tid] ?? null) as (string | null);
              return (
                <div key={tid} className="team-card">
                  <div className="row">
                    <label className="label-strong">{effectiveTeamNames[tid]} ({tid})</label>
                    <select
                      value={curSel || ""}
                      onChange={e => readonly ? null : setDef(tid, e.target.value || null)}
                      disabled={readonly}
                    >
                      <option value="">— 선택 안 함 —</option>
                      {options.map(opt => (
                  ...
                  <div className="hint">팀당 1명에게 +2점이 부여됩니다.</div>
                </div>
              );
            })}
          </div>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .rank-card { position: relative; overflow: hidden; display:flex; align-items:center; background:#1c1c1f; border-radius:10px; padding:6px 8px; color:var(--text); border:1px solid #24262c; gap:4px; }
          .rank-card::before {
            content: "";
            position: absolute;
            inset: 0;
            background-image: url('data:image/svg+xml;utf8,%3Csvg%20xmlns%3D%22http%3A//www.w3.org/2000/svg%22%20width%3D%22400%22%20height%3D%22120%22%3E%0A%3Cdefs%3E%3ClinearGradient%20id%3D%22g%22%20x1%3D%220%22%20y1%3D%220%22%20x2%3D%221%22%20y2%3D%221%22%3E%0A%3Cstop%20offset%3D%220%22%20stop-color%3D%22%23d4af37%22/%3E%3Cstop%20offset%3D%221%22%20stop-color%3D%22%238e793e%22/%3E%3C/linearGradient%3E%3C/defs%3E%0A%3Crect%20x%3D%220%22%20y%3D%220%22%20width%3D%22400%22%20height%3D%22120%22%20fill%3D%22url%28%23g%29%22/%3E%0A%3Cpath%20d%3D%22M0%2C90%20L300%2C120%20L400%2C120%20L400%2C0%20L0%2C0%20Z%22%20fill%3D%22rgba%280%2C0%2C0%2C0.15%29%22/%3E%0A%3Ctext%20x%3D%2216%22%20y%3D%2228%22%20font-size%3D%2218%22%20fill%3D%22%23fff%22%20font-weight%3D%22700%22%3EFIFA%20STYLE%3C/text%3E%0A%3C/svg%3E');
