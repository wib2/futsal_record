...
function emptySession(): Session {
  return { rosters: { A:[], B:[], C:[] }, matches: [], matchStats: {}, defAwards: { A:null, B:null, C:null }, notes:"" };
}
function ensureSession(map: Record<string, Session>, date: string): Session {
  const key = ensureSunday(date);
...
function computeStandings(matchesInput: Match[]|null|undefined): StandingRow[] {
  ...
  const tstats: Record<TeamId, StandingRow> = {
  ...
  for (const m of matches){
    ...
    tstats[m.home].gf += HG; tstats[m.home].ga += AG;
    tstats[m.away].gf += AG; tstats[m.away].ga += HG;
    if (HG>AG){ tstats[m.home].pts+=3; tstats[m.home].w++; tstats[m.away].l++; }
    else if (HG<AG){ tstats[m.away].pts+=3; tstats[m.away].w++; tstats[m.home].l++; }
    else { tstats[m.home].pts++; tstats[m.away].pts++; tstats[m.home].d++; tstats[m.away].d++; }
  }
  for (const t of TEAM_IDS) tstats[t].gd = tstats[t].gf - tstats[t].ga;
  return Object.values(tstats).sort((a,b)=> b.pts-a.pts || b.gd-a.gd || b.gf-a.gf || a.team.localeCompare(b.team));
}
function computeTeamBonus(st: StandingRow[]): Record<TeamId, number>{
  const order = st.map(s=>s.team);
  const out: Record<TeamId, number> = { A:0, B:0, C:0 };
  order.forEach((tid,i)=> out[tid] = i===0?4 : i===1?2 : 1);
  return out;
}
...
export default function App(){
  ...
  const initial: PersistShape = loaded ?? fallback;
  ...
  const cur: Session = useMemo(()=> ensureSession(sessionsByDate, sessionDate), [sessionsByDate, sessionDate]);

  const patchSession = (patch: Partial<Session>) => {
    if (readonly) return;
    setSessionsByDate(prev => {
      const k = ensureSunday(sessionDate);
      const base = ensureSession(prev, k);
      return { ...prev, [k]: { ...base, ...patch } as Session };
    });
  };
  ...
  const addPlayer = (name: string) => {
    ...
    if (players.some(p => p.name === nm)) { alert("이미 있는 이름입니다"); return; }
  ...
  const setMatchStat = (mid: string, pid: string, field: "goals"|"assists"|"cleansheets", value: number) => {
    if (readonly) return;
    const row = { ...(cur.matchStats?.[mid] || {}) } as MatchStats;
    const curv = row[pid] || { goals:0, assists:0, cleansheets:0 };
  ...
  const changeDate = (v: string) => {
    const sunday = ensureSunday(v);
    setSessionDate(sunday);
    if (readonly) return;
    setSessionsByDate(prev => ({ ...prev, [sunday]: ensureSession(prev, sunday) }));
  };
  ...
  function calcScores(session: Session){
    ...
    asArray(session.matches, []).forEach(m => {
      ...
      Object.entries(ms).forEach(([pid, s]) => {
        ...
        const cc = asNumber((s as any).cleansheets,0);
        out[pid] = { goals: base.goals+gg, assists: base.assists+aa, cleansheets: base.cleansheets+cc, team };
      });
    ...
    TEAM_IDS.forEach(tid => asArray(session.rosters[tid], []).forEach(pid => {
      if (!out[pid]) out[pid] = { goals:0, assists:0, cleansheets:0, team: tid };
    }));

    Object.keys(out).forEach(pid => {
      const team = out[pid].team as TeamId | "-";
      const def = team!=="-" && (cur.defAwards?.[team]||null)===pid ? 2 : 0;
      const tb = team!=="-" ? (teamBonus[team]||0) : 0;
      const total = out[pid].goals + out[pid].assists + out[pid].cleansheets + def + tb;
      out[pid] = {
        ...out[pid],
        def, teamBonus: tb, total,
  ...
  async function setPin(){
    ...
    setPinHash(h); sessionStorage.setItem(SS_PIN_AUTHED,'1'); setAuthed(true); setPinInput('');
  }
  async function unlock(){
    if (!pinInput) return alert("PIN을 입력하세요");
    const h = await sha256Hex(pinInput);
    if (h === pinHash){ sessionStorage.setItem(SS_PIN_AUTHED,'1'); setAuthed(true); setPinInput(''); }
    else alert("PIN 불일치");
  }
  ...
  return (
    <div style={{ maxWidth: 1200, margin:"0 auto", padding: 16, fontFamily: 'system-ui, -apple-system, Segoe UI, Roboto, \"Noto Sans KR\", sans-serif' }}>
      ...
      <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap', marginBottom:12, padding:10, border:'1px dashed #bbb', borderRadius:8, background:'#fafafa' }}>
        ...
        {!pinHash ? (<>
          <span style={{ fontWeight:700 }}>관리자 PIN 설정:</span>
          <input type="password" value={pinInput} onChange={e=>setPinInput(e.target.value)} placeholder="새 PIN" />
          <button onClick={setPin}>설정</button>
          <span style={{ fontSize:12, color:'#666' }}>※ 최초 1회만 설정하면 됩니다.</span>
        </>) : authed ? (<>
          <span style={{ color:'#0a0', fontWeight:700 }}>관리자 모드</span>
          <button onClick={lock}>잠금</button>
          <button onClick={copyViewerLink}>보기 전용 링크 복사</button>
          <span style={{ marginLeft:8, fontSize:12, color: supaEnabled()? '#0a0':'#a00' }}>실시간: {supaEnabled()? '켜짐(환경변수 설정됨)':'꺼짐(환경변수 없음)'}</span>
        </>) : (<>
          <span style={{ fontWeight:700 }}>관리자 PIN:</span>
      ...
      <section style={box}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:8 }}>
          {TEAM_IDS.map(tid => (
            <div key={tid} style={{ border:'1px solid #ddd', borderRadius:8, padding:8 }}>
              ...
              <div style={{ maxHeight:200, overflow:'auto' }}>
                {players.filter(p=>p.active).map(p => (
                  <label key={p.id} style={{ display:'flex', gap:6, alignItems:'center', fontSize:14 }}>
      ...
      <section style={box}>
        ...
        <div style={{ display:'grid', gap:6 }}>
          {asArray(cur.matches, []).map(m => (
            <MatchRow key={m.id}
              ...
              players={players} values={cur.matchStats?.[m.id] || {}}
        ...
        <div style={{ marginTop:12, overflowX:'auto' }}>
          ...
          <table style={table}>
            <thead><tr><th>순위</th><th>팀</th><th>승점</th><th>승</th><th>무</th><th>패</th><th>득점</th><th>실점</th><th>득실</th><th>팀</th></tr></thead>
            <tbody>
              {computeStandings(cur.matches).map((t,idx)=> (
                <tr key={t.team}>
                  <td>{idx+1}</td>
                  <td>{teamNames[t.team]} <span style={{ color:"#888" }}>({t.team})</span></td>
                  <td>{t.pts}</td><td>{t.w}</td><td>{t.d}</td><td>{t.l}</td><td>{t.gf}</td><td>{t.ga}</td><td>{t.gd}</td>
                  <td style={{ fontWeight:700 }}>{computeTeamBonus(computeStandings(cur.matches))[t.team]}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section style={box}>
        <h3>비고 / 메모</h3>
        <textarea value={cur.notes} onChange={e=>setNotes(e.target.value)} placeholder="예: 특이사항 등" style={{ width:"100%", minHeight:80 }} disabled={readonly} />
      </section>
...
function MatchRow({ m, readonly, updateMatch, deleteMatch, rosterA, rosterB, players, values, onChange }: {
  m: Match; readonly: boolean;
  updateMatch: (id:string, patch: Partial<Match>)=>void;
  deleteMatch: (id:string)=>void;
  rosterA: string[]; rosterB: string[]; players: Player[];
  values: MatchStats;
  onChange: (pid:string, field:"goals"|"assists"|"cleansheets", value:number)=>void;
}){
    ...
    <div style={{ border:"1px solid #eee", borderRadius:8, padding:8 }}>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 60px 20px 60px 1fr auto", gap:6, alignItems:"center" }}>
        ...
        <div style={{ display:"flex", gap:6 }}>
          ...
          <button onClick={()=> deleteMatch(m.id)} style={{ color:"#a00" }} disabled={readonly}>삭제</button>
        </div>
      </div>
      <div style={{ display:"flex", gap:10, alignItems:"center", marginTop:6 }}>
        <div>
          <div style={{ fontSize:12, color:"#666" }}>홈 GK</div>
        ...
        <div>
          <div style={{ fontSize:12, color:"#666" }}>원정 GK</div>
      ...
      {open && (
        <div style={{ marginTop:8 }}>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:8 }}>
            {[...(rosterA||[]), ...(rosterB||[])].map(pid => {
              const v = values[pid] || { goals:0, assists:0, cleansheets:0 };
              return (
