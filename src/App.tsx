...
useEffect(() => {
  ...
  function calcScores(session: Session) {
    ...
    const teamBonusByTeam = computeTeamBonus(standings.filter(r => ACTIVE_TEAMS.includes(r.team)), (session as any)?.hasTeamD || false);
            ...
            <div className="table-wrap">
              ...
              <table className="tbl"><thead><tr>
                ...
                {computeStandings(cur.matches).map((t, idx) => (
                  <tr key={t.team}><td>{idx + 1}</td><td>{effectiveTeamNames[t.team]} <span className="subtle">({t.team})</span></td><td>{t.pts}</td><td>{t.w}</td><td>{t.d}</td><td>{t.l}</td><td>{t.gf}</td><td>{t.ga}</td><td>{t.gd}</td><td className="bold">{(Array.isArray(cur.matches) && cur.matches.length>0 ? computeTeamBonus(computeStandings(cur.matches).filter(r => ACTIVE_TEAMS.includes(r.team)), cur?.hasTeamD || false)[t.team] : 0)}</td></tr>
                ))}
              </tbody></table>
            </div>
          </section>
