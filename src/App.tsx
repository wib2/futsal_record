...
const UniformIcon: React.FC<{ fill: string; size: number; stroke?: string }> = ({ fill, size, stroke = "#111" }) => (
  // width를 1.25배로 넓혀서 이름 가독성 향상 (라운드넥)
...
/* ====== 포메이션 컴포넌트 ====== */
const FORMATION_POINTS: Record<FormationKey | "1-2" | "2-2-1", { x: number; y: number; label: string }[]> = {
  /* 좌우 0~100, 상단 상대 진영 (GK 제외한 필드 좌표만 정의) */
  "1-2": [
    { x: 50, y: 40, label: "FW" },
  ...
  "3-1": [
    { x: 20, y: 36, label: "FW" }, { x: 50, y: 36, label: "FW" }, { x: 80, y: 36, label: "FW" },
    { x: 50, y: 78, label: "DF" },
  ],
  "2-2-1": [
    { x: 30, y: 28, label: "FW" }, { x: 70, y: 28, label: "FW" },
    { x: 30, y: 56, label: "MF" }, { x: 70, y: 56, label: "MF" },
    { x: 50, y: 82, label: "DF" }
  ]
};
...
function FormationPreview({
  ...
  const includeGK = gkList.length === 1 ? gkList[0] : null;

  // 필드 인원 기반 자동 포메이션
  const fCount = fields.length;
  const autoForm = (fCount <= 3) ? ("1-2" as const)
                  : (fCount === 4) ? ("1-2-1" as const)
                  : ("2-2-1" as const);
  ...
  return (
    <div className="formation-card">
      ...
      <svg viewBox="0 0 100 140" className="pitch">
        ...
        {/* players */}
        {coords.map((pt, i) => {
          const pid = fields[i] || null;
          const name = pid ? (players.find(p => p.id === pid)?.name || "?") : "";
          return (
            <g key={i} transform={`translate(${pt.x}, ${pt.y})`}>
              <UniformIcon fill={jerseyFill} size={14} />
              <text x={7.8125} y={7} textAnchor="middle" dominantBaseline="middle" fontWeight={800} fontSize={7 * 0.9} fill="#111">
                {pid ? tail2(name) : "?"}
              </text>
            </g>
          );
      ...
      {roster.length > fields.length && (
        <div className="bench">벤치: {roster.filter(id => !fields.includes(id)).map(id => (players.find(p => p.id === id)?.name || "?")).join(", ")}</div>
      )}
    </div>
  );
}
