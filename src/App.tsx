...
function FormationPreview({
  ...
  const chosen = ((gkList.length === 1 ? [gkList[0]] : []) as string[]).concat(fields).slice(0, 5) as string[];

  /* 좌표: GK + 4 */
  const pts = FORMATION_POINTS[formation];
  const coords = (gkList.length === 1) ? pts : pts.slice(1);
    ...
    <div className="formation-card">
      ...
      <svg viewBox="0 0 100 140" className="pitch">
        ...
        {coords.map((pt, i) => {
          ...
          const init = pid ? (name.slice(-2)) : "?";
          // name size adapt to jersey width (heuristic)
          const nameSize = Math.max(2.4, 5.4 - (name.length * 0.25));
          return (
            <g key={i} transform={`translate(${pt.x}, ${pt.y})`}>
              <g transform="translate(0,0) scale(1.9)" className="jersey">
                <path d="M -5,-6 L -1,-6 L 0,-5 L 1,-6 L 5,-6 L 4,-1 L 3,0 L 3,6 L -3,6 L -3,0 L -4,-1 Z" fill={jerseyFill} stroke="var(--jersey-stroke)" strokeWidth="0.6"/>
                <text y="-0.5" textAnchor="middle" dominantBaseline="central" className="player-initials">{init}</text>
              </g>
              <text y="8.5" textAnchor="middle" className="player-label" style={{fontSize: `${nameSize}px`}}>{pid ? name : "빈 자리"}</text>
            </g>
          );
        })}
