 ...
 * - 최신 경기(큰 seq) 위로 정렬
 * - Supabase Realtime 동기화(깜빡임/루프 방지)
 * - 가나다 정렬, GK 하단 정렬
 * - 모바일 UI 보강 (버튼/입력칸 넘침 방지)
 */
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (ready && hydrated) { if (!deepEqualState(cloud, s)) setCloud(s); }
  ...
  return (<div className="wrap">
    ...
    <style>{`
      *, *::before, *::after { box-sizing: border-box; }
      ...
      .scoreline select, .scoreline input[type="number"] { min-width: 0; } /* 넘침 금지 */
      .scoreline input[type="number"] { text-align:center; }
      .colon { text-align:center; font-weight:700; }
      .head-actions { display:flex; gap:6px; justify-content:flex-end; flex-wrap: nowrap; }
      .danger { color:#a00; }
      ...
      @media (max-width: 430px) {
        ...
        .seq { order: 1; }
        .scoreline { order: 2; grid-template-columns: minmax(0,1fr) 56px 14px 56px minmax(0,1fr); }
        .head-actions { order: 3; justify-content: flex-start; flex-wrap: wrap; }
        .head-actions button { padding: 6px 10px; }
        .gk-row { gap: 8px; }
      }

      /* 극소폭(iPhone SE 등) 추가 보정 */
      @media (max-width: 360px) {
        .scoreline { grid-template-columns: minmax(0,1fr) 50px 12px 50px minmax(0,1fr); }
        .stat-input { width: 58px; }
      }

      /* 더 좁은 화면에서 테이블 여백 최소화 */
