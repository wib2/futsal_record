...
type Props = {
  players: Player[];
  /** true이면 읽기 전용(관리자 아님). 기본값: false */
  readonly?: boolean;
};

/** Supabase client: 전역 또는 Vite 환경변수 기반 생성 */
...
function getSupabase(): SupabaseClient | null {
  ...
  if (!url || !key) {
    console.warn("[FifaSection] Missing env. VITE_SUPABASE_URL/ANON_KEY.");
    return null;
  }
...
export default function FifaSection({ players, readonly = false }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (alive) {
        if (row) { setForm(fromDbRow(row)); }
        else { const nm = players.find(p=>p.id===pid)?.name || ""; setForm(f => ({ ...f, name: nm })); }
        setLoading(false);
      }
    })();
    return () => { alive = false; };
  ...
  async function save() {
    ...
    alert(ok ? "피파 카드 능력치를 Supabase에 저장했습니다." : "저장 실패(콘솔 확인)");
  }
  async function download() {
    if (svgRef.current) await downloadSvgAsPng(svgRef.current, `${form.name || "player"}-fifa-card.png`);
  }

  // 현재 값이 목록에 없을 때도 유지되도록 helper
  const ensureOption = (val?: string) => {
    const v = val || "";
    if (!v) return null;
    if (!foundImages.includes(v)) {
      const label = v.split("/").slice(-1)[0] || v;
      return <option key={`current-${v}`} value={v}>{label}</option>;
    }
    return null;
  };
  ...
  return (
    <section className="box">
      {/* 모바일 대응 CSS + 오버플로 방지 */}
      <style>{`
        .fifa-flex { display:flex; align-items:flex-start; gap:24px; flex-wrap:wrap; width:100%; }
        .fifa-card-col { flex: 1 1 280px; min-width: 260px; max-width: 420px; }
        .fifa-form-grid { display:grid; grid-template-columns: 120px 1fr; gap: 8px 12px; flex: 1 1 260px; min-width: 260px; width:100%; }
        .fifa-form-grid > * { min-width: 0; max-width: 100%; }
        .fifa-form-grid input, .fifa-form-grid select { width: 100%; box-sizing: border-box; max-width: 100%; }
        @media (max-width: 640px) {
          .fifa-form-grid { grid-template-columns: 1fr; }
          .fifa-form-grid label { margin-top: 10px; }
          .fifa-card-col { min-width: 0; } /* 아주 작은 화면에서 강제 줄이기 */
        }
      `}</style>
      ...
      <div className="row" style={{ gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <button onClick={save} disabled={!canEdit}>저장(Supabase)</button>
        ...
        <div className="fifa-form-grid">
          ...
          {foundImages.length > 0 && !readonly && <>
            ...
            >
              ...
              {ensureOption(form.photoUrl)}
              {foundImages.map(u=> <option key={u} value={u}>{u.split("/").slice(-1)[0]}</option>)}
            </select>

            <label>엠블럼 선택</label>
            <select
              value={form.clubCrest || ""}
              onChange={(e)=> set("clubCrest", e.target.value)}
              disabled={!canEdit}
            >
              <option value="" disabled>— 파일 선택 —</option>
              {ensureOption(form.clubCrest)}
              {foundImages.map(u=> <option key={u} value={u}>{u.split("/").slice(-1)[0]}</option>)}
            </select>

            <label>국기 선택</label>
            <select
              value={form.nationFlag || ""}
              onChange={(e)=> set("nationFlag", e.target.value)}
              disabled={!canEdit}
            >
              <option value="" disabled>— 파일 선택 —</option>
              {ensureOption(form.nationFlag)}
              {foundImages.map(u=> <option key={u} value={u}>{u.split("/").slice(-1)[0]}</option>)}
            </select>
          </>}
        </div>
