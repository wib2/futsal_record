...
type Props = {
  players: Player[];
  readonly?: boolean; // 분석 탭 등 비관리자는 true로 전달
};
...
try {
  // 경고가 떠도 기존 옵션(as:'url')은 동작합니다. (기능 변경 없음)
    ...
    <section className="box">
      ...
      <div className="fifa-flex">
        ...
        <div className="fifa-form-grid">
          ...
          {/* URL 입력칸 제거 */}
          {/* <label>국기 URL</label> ... */}
          {/* <label>클럽 엠블럼 URL</label> ... */}
          {/* <label>사진 URL</label> ... */}

          {/* 프로젝트 이미지 선택(자동 탐색): 관리자만 선택 가능 (readonly면 비활성화) */}
          {foundImages.length > 0 && <>
            <label>선수 사진 선택</label>
            <select onChange={(e)=>{ const url = e.target.value; if (url) set("photoUrl", url); }} defaultValue="" disabled={readonly}>
              <option value="" disabled>— 파일 선택 —</option>
              {foundImages.map(u=> <option key={u} value={u}>{u.split("/").slice(-1)[0]}</option>)}
            </select>

            <label>엠블럼 선택</label>
            <select onChange={(e)=>{ const url = e.target.value; if (url) set("clubCrest", url); }} defaultValue="" disabled={readonly}>
              <option value="" disabled>— 파일 선택 —</option>
              {foundImages.map(u=> <option key={u} value={u}>{u.split("/").slice(-1)[0]}</option>)}
            </select>

            <label>국기 선택</label>
            <select onChange={(e)=>{ const url = e.target.value; if (url) set("nationFlag", url); }} defaultValue="" disabled={readonly}>
              <option value="" disabled>— 파일 선택 —</option>
              {foundImages.map(u=> <option key={u} value={u}>{u.split("/").slice(-1)[0]}</option>)}
            </select>
          </>}
