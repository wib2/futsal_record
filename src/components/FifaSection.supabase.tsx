...
// --- Supabase client dynamic loader ---
...
async function loadFifaByPlayer(player_id: string): Promise<DBRow | null> {
  ...
  if (error) { console.warn('[FifaSection] load error', error); }
  return data;
}

async function upsertFifa(row: DBRow): Promise<boolean> {
  const sb = await loadSupabaseClient(); if (!sb) { alert('Supabase 클라이언트를 찾지 못했습니다.'); return false; }
  const { error } = await sb.from('fifa_cards').upsert(row, { onConflict: 'player_id' });
  if (error) { console.error('[FifaSection] upsert error', error); alert('저장 실패: ' + (error.message || 'unknown')); return false; }
  return true;
} = await sb.from("fifa_cards").select("id").eq("player_id", row.player_id).maybeSingle();
